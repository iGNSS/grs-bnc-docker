...
bncWindow::bncWindow() {
  ...
  // Upload Results
  // -------------
  _uploadTable = new QTableWidget(0,13);
  _uploadTable->setHorizontalHeaderLabels(QString("Host, Port, Mount, Password, System, Format, CoM, SP3 File, RNX File, PID, SID, IOD, bytes").split(","));
  ...
  _uploadTable->horizontalHeader()->resizeSection( 5,11*ww);
  _uploadTable->horizontalHeader()->resizeSection( 6, 4*ww);
  ...
  _uploadTable->horizontalHeader()->resizeSection(12,12*ww);
...
void bncWindow::slotDelUploadRow() {
  ...
  for (int iRow = 0; iRow < _uploadTable->rowCount(); iRow++) {
    BNC_CORE->_uploadTableItems[iRow] =
                                (bncTableItem*) _uploadTable->item(iRow, 12);
  }
